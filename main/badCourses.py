...
#				print 'lab if'
				lab = Lab.objects.get_or_create(course=lastCourse, title=title)
				is_new = lab[1]
#				print lab
				lab = lab[0]
				if not is_new:
...
#		print 'building: %s' % building 
		...
		course = Course.objects.get_or_create(title=title, number=num, section_number=section, abbr=abbr)
		isNew = course[1]
